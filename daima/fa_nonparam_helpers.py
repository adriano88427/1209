...
from daima.fa_config import RELIABILITY_CONFIG, SEGMENT_MIN_SAMPLES
from daima.fa_stat_utils import bounded_normalize
...
def _fa_get_scoring_standards(self):
    ...
    """
    standards = []
    standards.append("本报告采用分维度评分 + 动态可靠性加权机制，具体计算规则如下：")
    standards.append("")
    standards.append("一、基础表现（日度 IC / IR / 统计显著性）")
    standards.append("  • IC 均值：|IC|≥0.05 记 100，0.03 记 90，0.02 记 75，0.015 记 60，0.01 记 45，其余 20；")
    standards.append("  • IR 值：|IR|≥1.5 记 100，1.0 记 90，0.7 记 80，0.5 记 70，0.3 记 55，0.1 记 40；")
    standards.append("  • 统计显著性：p<0.01 记 100，0.01~0.05 记 85，0.05~0.1 记 70，≥0.1 记 40；")
    standards.append("  • 多空收益/胜率：收益≥4% 记 100，3% 记 90，2% 记 80；胜率≥65% 记 100，50% 记 65。最终基础得分 = 40%*IC + 20%*IR + 20%*p 值 + 20%*(收益/胜率均值)。")
    standards.append("")
    standards.append("二、整体样本表现（整体 IC / IR / p 值 / 样本量）")
    standards.append("  • 输入来自 `extra_stats['overall_metrics']`，按 Spearman/Pearson 全样本计算；")
    standards.append("  • 整体 IC 与 IR 使用与基础维度相同的刻度；")
    standards.append("  • p 值同样按显著性阶梯赋分；")
    standards.append("  • 样本量评分：样本≥800 记 100，600 记 95，400 记 90，300 记 85，200 记 75，120 记 65，80 记 55，40 记 45。综合得分 = 45%*整体 IC + 20%*整体 IR + 15%*p 值 + 10%*多空收益 + 10%*样本量。")
    standards.append("")
    standards.append("三、滚动稳定度（Rolling IC）")
    standards.append("  • 对每个窗口记录 CV、半衰期、MAIC，分别按 `_score_cv/_score_half_life/_score_maic` 评分；")
    standards.append("  • CV≤0.3 记 100，0.6 记 60，>0.8 记 20；半衰期≥20 记 100，≤5 记 40；MAIC≥0.05 记 100，0.02 记 65；")
    standards.append("  • 多窗口得分求平均获得滚动稳定得分。")
    standards.append("")
    standards.append("四、时序一致性")
    standards.append("  • Lag1 自相关最佳区间 0.4~0.8，偏离区间扣分；")
    standards.append("  • 趋势相关系数 |r|≤0.2 得高分，|r|>0.5 记 20；")
    standards.append("  • 换向次数 1~3 次最优，>6 次降至 45；")
    standards.append("  • 结合均值回归强度、排名波动等指标平均后得到时序一致性得分。")
    standards.append("")
    standards.append("五、样本稳健性（抽样敏感度）")
    standards.append("  • 80%/90%/100% 抽样的 IC 均值、标准差、IQR、成功率分别打分：std≤0.003→100，0.01→65，>0.02→35；成功率≥95%→100，<80%→35；")
    standards.append("  • 跨样本方差≤0.01→95，0.02→60，>0.05→30；")
    standards.append("  • 上述得分求平均得到样本稳健性。")
    standards.append("")
    standards.append("六、动态权重与可靠性")
    standards.append("  • 名义权重为基础35% / 整体25% / 滚动15% / 时序12.5% / 样本12.5%；")
    standards.append("  • 每个维度计算 0.3~1.0 的可靠性（依据日均样本、缺样、Top5 集中度、整体样本数、滚动CV 等）；")
    standards.append("  • `final_weight_i = base_weight_i * clamp(reliability_i, 0.4~1.6)`，若可靠性 <0.35 则该维度不参与评分；")
    standards.append("  • 归一化并限制在 5%~55% 区间后写入报告“评分权重调整”一节。")
    standards.append("")
    standards.append("七、综合评分与评级")
    standards.append("  • 最终得分 = Σ(维度得分 * 动态权重)，默认输出保留两位小数；")
    standards.append("  • 评级映射：A+ ≥90，A ≥85，A- ≥75，B+ ≥65，B ≥55，C+ ≥45，否则 C；")
    standards.append("  • 报告中同时给出正/负向因子的建议权重（基于评级）以及权重调整说明。")
    standards.append("")
    standards.append("八、低样本提示")
    standards.append("  • 若日度样本触发低样本模式（avg<4 或缺样>20%），报告会说明“低样本量模式，窗口=3日，min样本=2”，并自动上调整体/样本稳健性的权重；")
    standards.append("  • 若滚动/时序可靠性为“低”，会在详解段落提示“稳健性权重不足，建议谨慎使用”。")
    standards.append("")
    standards.append("九、投资建议提示")
    standards.append("  • A± 因子建议 10%~25% 权重，B 级 3%~10%，C 级仅作辅助或对冲；")
    standards.append("  • 负向因子按反向 IC 的绝对值评分，报告会标注“建议权重(反向)”及反向胜率。")
...
def _fa_generate_factor_classification_overview(self):
    ...
    if len(positive_factors) > 0:
        ...
        overview_lines.append(
            f"最佳正向因子: {best_positive_factor['因子名称']} (IC均值: {best_positive_factor['IC均值']:.3f}, 评级: {best_positive_factor['评级']})"
        )
    else:
    ...
    if len(negative_factors) > 0:
        ...
        overview_lines.append(
            f"最佳负向因子: {best_negative_factor['因子名称']} (IC均值: {best_negative_factor['IC均值']:.3f}, 评级: {best_negative_factor['评级']})"
        )
    else:
    ...
    overview_lines.append("\n策略建议:")
    if len(positive_factors) > 0:
        overview_lines.append("正向因子策略建议:")
        top_positive = positive_factors.head(3)
        for factor in top_positive.to_dict('records'):
            overview_lines.append(
                f"  - {factor['因子名称']}: 建议权重{_fa_get_suggested_weight(self, factor['评级'], True)} (IC均值: {factor['IC均值']:.3f}, IR值: {factor['IR值']:.3f})"
            )
    if len(negative_factors) > 0:
        overview_lines.append("负向因子策略建议:")
        top_negative = negative_factors.head(3)
        for factor in top_negative.to_dict('records'):
            overview_lines.append(
                f"  - {factor['因子名称']}: 建议权重{_fa_get_suggested_weight(self, factor['评级'], False)} (IC均值: {factor['IC均值']:.3f}, IR值: {factor['IR值']:.3f})"
            )

    overview_lines.append("\n组合配置建议:")
    if len(positive_factors) > 0 and len(negative_factors) > 0:
        overview_lines.append("1. 正向因子+负向因子对冲策略（建议使用3-5个正向因子搭配2-3个负向因子）")
        overview_lines.append("2. 保持因子风格多元，避免集中在单一风格或行业")
    elif len(positive_factors) > 0:
        overview_lines.append("1. 仅使用正向因子构建多因子组合")
