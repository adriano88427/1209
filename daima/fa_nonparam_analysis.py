...
class FactorAnalysis:
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.data is not None:
                print(f"[INFO] 数据加载成功: {len(self.data)} 行, 列数 {self.data.shape[1]}")
            return True
        except Exception as e:
            print(f"[ERROR] 数据加载失败: {e}")  # 保留错误提示
            return False
    ...
    def generate_auxiliary_analysis_report(
        ...
        if summary_rows:
            ...
            summary_csv = f"辅助分析报告摘要_{timestamp}.csv"
            summary_csv_path = build_report_path(summary_csv)
            summary_df.to_csv(summary_csv_path, index=False, encoding='utf-8-sig')
            print(f"[OK] 辅助分析摘要已生成: {summary_csv_path}")

        self.auxiliary_stats = auxiliary_stats

        if score_rows:
            score_df = pd.DataFrame(score_rows)
            score_csv = f"辅助分析稳健性评分_{timestamp}.csv"
            score_csv_path = build_report_path(score_csv)
            score_df.to_csv(score_csv_path, index=False, encoding='utf-8-sig')
            print(f"[OK] 辅助稳健性评分摘要已生成: {score_csv_path}")
