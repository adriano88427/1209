...
## 6. 输出与审计
- 主输出：`因子分析汇总_*.csv`、HTML/XLSX 报告。  
- 验证输出：`ic_diff.csv`、`ic_stats_compare.csv`、`audit_summary_latest.csv`。  
- 基准对照：`ic_benchmark_external_pandas.csv`、`ic_benchmark_compare_pandas.csv`。  
- 自校验：`ic_selfcheck_compare.csv`（需日度落盘）。  
- CI：`ic_bootstrap_ci.csv`（可选）。
